...
select city_name , round(
(population * 0.25)/1000000,2) as coffee_consumers_in_millions, city_rank
from city
order by 2 desc
...
SELECT 
  ...
  ct.avg_sale_pr_cx,
  ROUND(
    cr.estimated_rent * 1.0 / 
    ct.total_cx, 2) as avg_rent_per_cx
...
SELECT
  ...
  last_month_sale,
  ROUND(
    (cr_month_sale - last_month_sale) * 1.0 / last_month_sale * 100
    , 2
  ) as growth_ratio
FROM growth_ratio
WHERE 
...
WITH city_table AS (
  SELECT 
    ...
    ROUND(
      SUM(s.total) * 1.0 /
      COUNT(DISTINCT s.customer_id) * 1.0
    , 2) as avg_sale_pr_cx
  FROM sales as s
  JOIN customers as c
  ON s.customer_id = c.customer_id
